...
export function add(a: number | string, b: number | string): number | string {
  if (typeof a === 'number' && typeof b === 'number') {
    ...
    const fnType = t.resolveReference(fnSymbol.valueDeclarationType);
    expect(fnType.text).to.eq('{ (a: string, b: string): string; (a: number, b: number): number; }');
    ...
    expect(fnType.callSignatures![1].parameters!.map((p) => t.resolveReference(p)).filter(isDefined).map((s) => ({
      name: s.text || s.name,
      type: t.resolveReference(s.valueDeclarationType).text
    }))).to.deep.eq([{
      name: 'a', type: 'number'
...
export function add(a: number | string, b: number | string): number | string {
  ...
  @test
  public async 'function with multiple signatures, with a doc override for several signatures'(): Promise<void> {
    const code = `
/**
