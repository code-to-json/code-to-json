exports['SerializationSnapshotTests Class with implied constructor 1'] = {
  "types": {
    "T01m4wn8d6tuf": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wn8d6tuf",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "SimpleClass": [
          "symbol",
          "S01m4wn665eqo"
        ]
      }
    },
    "T01m4wnt3kk7k": {
      "typeString": "typeof SimpleClass",
      "entity": "type",
      "id": "T01m4wnt3kk7k",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {},
      "constructorSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmsic0ho"
          ],
          "typeString": "(): SimpleClass"
        }
      ]
    },
    "T01m4wmsic0ho": {
      "typeString": "SimpleClass",
      "entity": "type",
      "id": "T01m4wmsic0ho",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Class",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmsic0ho"
      ],
      "thisType": [
        "type",
        "T01m4wmy50x4t"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmsic0ho1"
        ]
      ]
    },
    "T01m4wmy50x4t": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wmy50x4t",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wmsic0ho1"
      ]
    },
    "T01m4wmsic0ho1": {
      "typeString": "SimpleClass",
      "entity": "type",
      "id": "T01m4wmsic0ho1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmsic0ho"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmsic0ho"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmy50x4t"
        ]
      ]
    }
  },
  "symbols": {
    "S01m4wnncslv4": {
      "id": "S01m4wnncslv4",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wn8d6tuf"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "SimpleClass": [
          "symbol",
          "S01m4wn665eqo"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyhu5"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyhu5"
        ],
        1,
        1,
        1,
        28
      ]
    },
    "S01m4wn665eqo": {
      "id": "S01m4wn665eqo",
      "entity": "symbol",
      "name": "SimpleClass",
      "flags": [
        "Class"
      ],
      "type": [
        "type",
        "T01m4wnt3kk7k"
      ],
      "symbolString": "SimpleClass",
      "typeString": "typeof SimpleClass",
      "modifiers": [
        "export"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyhu5"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyhu5"
        ],
        1,
        1,
        1,
        28
      ]
    }
  }
}

exports['SerializationSnapshotTests Class with properties and methods 1'] = {
  "types": {
    "T01m4wnvl28mi": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wnvl28mi",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "SimpleClass": [
          "symbol",
          "S01m4wmy6qxnn"
        ]
      }
    },
    "T01m4wlocrpyg": {
      "typeString": "typeof SimpleClass",
      "entity": "type",
      "id": "T01m4wlocrpyg",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {},
      "constructorSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wnosh4qq"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlnrhn38"
            ]
          ],
          "typeString": "(bar: string): SimpleClass"
        }
      ]
    },
    "T01m4wmr2p2yx": {
      "typeString": "string",
      "entity": "type",
      "id": "T01m4wmr2p2yx",
      "flags": [
        "String"
      ],
      "primitive": true
    },
    "T01m4wmt920g7": {
      "typeString": "(x: number[]) => number",
      "entity": "type",
      "id": "T01m4wmt920g7",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p302"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmgai7ni"
            ]
          ],
          "typeString": "(x: number[]): number"
        }
      ]
    },
    "T01m4wnosh4qq": {
      "typeString": "SimpleClass",
      "entity": "type",
      "id": "T01m4wnosh4qq",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Class",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wnosh4qq"
      ],
      "properties": {
        "foo": [
          "symbol",
          "S01m4wm5niuah"
        ],
        "baz": [
          "symbol",
          "S01m4wmy01yu1"
        ]
      },
      "thisType": [
        "type",
        "T01m4wn5xna84"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wnosh4qq1"
        ]
      ]
    },
    "T01m4wmr2p302": {
      "typeString": "number",
      "entity": "type",
      "id": "T01m4wmr2p302",
      "flags": [
        "Number"
      ],
      "primitive": true
    },
    "T01m4wn5xna84": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wn5xna84",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wnosh4qq1"
      ]
    },
    "T01m4wnosh4qq1": {
      "typeString": "SimpleClass",
      "entity": "type",
      "id": "T01m4wnosh4qq1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wnosh4qq"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wnosh4qq"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wn5xna84"
        ]
      ]
    },
    "T01m4wlstx004": {
      "typeString": "number[]",
      "entity": "type",
      "id": "T01m4wlstx004",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmyry115"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmr2p302"
        ]
      ],
      "numberIndexType": [
        "type",
        "T01m4wmr2p302"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmyry115": {
      "typeString": "T[]",
      "entity": "type",
      "id": "T01m4wmyry115",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Interface",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmyry115"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "properties": {
        "length": [
          "symbol",
          "S01m4wlljkcrg"
        ],
        "toString": [
          "symbol",
          "S01m4wnclba08"
        ],
        "toLocaleString": [
          "symbol",
          "S01m4wlml65d9"
        ],
        "pop": [
          "symbol",
          "S01m4wnh2nepy"
        ],
        "push": [
          "symbol",
          "S01m4wm35aop6"
        ],
        "concat": [
          "symbol",
          "S01m4wnefiihw"
        ],
        "join": [
          "symbol",
          "S01m4wngkltv8"
        ],
        "reverse": [
          "symbol",
          "S01m4wmuy9l2g"
        ],
        "shift": [
          "symbol",
          "S01m4wmiyhx7n"
        ],
        "slice": [
          "symbol",
          "S01m4wlwouxwt"
        ],
        "sort": [
          "symbol",
          "S01m4wmgpbbip"
        ],
        "splice": [
          "symbol",
          "S01m4wmf5napj"
        ],
        "unshift": [
          "symbol",
          "S01m4wmatwspb"
        ],
        "indexOf": [
          "symbol",
          "S01m4wmuhwr2t"
        ],
        "lastIndexOf": [
          "symbol",
          "S01m4wmoc9yl5"
        ],
        "every": [
          "symbol",
          "S01m4wm17yufo"
        ],
        "some": [
          "symbol",
          "S01m4wmn5alt3"
        ],
        "forEach": [
          "symbol",
          "S01m4wlnp2rtw"
        ],
        "map": [
          "symbol",
          "S01m4wm6fmeva"
        ],
        "filter": [
          "symbol",
          "S01m4wlquekh0"
        ],
        "reduce": [
          "symbol",
          "S01m4wmlyhthq"
        ],
        "reduceRight": [
          "symbol",
          "S01m4wlm4esol"
        ]
      },
      "thisType": [
        "type",
        "T01m4wn7389oq"
      ],
      "numberIndexType": [
        "type",
        "T01m4wlxxxspc"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmyry1151"
        ]
      ]
    },
    "T01m4wlxxxspc": {
      "typeString": "T",
      "entity": "type",
      "id": "T01m4wlxxxspc",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wlxxxspc1"
        ]
      ]
    },
    "T01m4wn7389oq": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wn7389oq",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wmyry1151"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmyry1151": {
      "typeString": "T[]",
      "entity": "type",
      "id": "T01m4wmyry1151",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmyry115"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmyry115"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc1"
        ]
      ],
      "numberIndexType": [
        "type",
        "T01m4wlxxxspc1"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlmgaa33": {
      "typeString": "() => string",
      "entity": "type",
      "id": "T01m4wlmgaa33",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "typeString": "(): string"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm6hyl1v": {
      "typeString": "() => string",
      "entity": "type",
      "id": "T01m4wm6hyl1v",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "typeString": "(): string"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmbejkoe": {
      "typeString": "() => T",
      "entity": "type",
      "id": "T01m4wmbejkoe",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxspc"
          ],
          "typeString": "(): T"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlo5jwbl": {
      "typeString": "(...items: T[]) => number",
      "entity": "type",
      "id": "T01m4wlo5jwbl",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p302"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnh8h9vm"
            ]
          ],
          "typeString": "(...items: T[]): number"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm9hh1pr": {
      "typeString": "{ (...items: ConcatArray<T>[]): T[]; (...items: (T | ConcatArray<T>)[]): T[]; }",
      "entity": "type",
      "id": "T01m4wm9hh1pr",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmyry115"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnmco1l5"
            ]
          ],
          "typeString": "(...items: ConcatArray<T>[]): T[]"
        },
        {
          "returnType": [
            "type",
            "T01m4wmyry115"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wniiiax7"
            ]
          ],
          "typeString": "(...items: (T | ConcatArray<T>)[]): T[]"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnw50elx": {
      "typeString": "(separator?: string) => string",
      "entity": "type",
      "id": "T01m4wnw50elx",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wndu3pqv"
            ]
          ],
          "typeString": "(separator?: string): string"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnuhc3nk": {
      "typeString": "() => T[]",
      "entity": "type",
      "id": "T01m4wnuhc3nk",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmyry115"
          ],
          "typeString": "(): T[]"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnsu5n5g": {
      "typeString": "() => T",
      "entity": "type",
      "id": "T01m4wnsu5n5g",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxspc"
          ],
          "typeString": "(): T"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm3g2s0o": {
      "typeString": "(start?: number, end?: number) => T[]",
      "entity": "type",
      "id": "T01m4wm3g2s0o",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmyry115"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnkh8lv3"
            ],
            [
              "symbol",
              "S01m4wmhq6m6y"
            ]
          ],
          "typeString": "(start?: number, end?: number): T[]"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm5nwh0x": {
      "typeString": "(compareFn?: (a: T, b: T) => number) => T[]",
      "entity": "type",
      "id": "T01m4wm5nwh0x",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmyry115"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnsv0e1b"
            ]
          ],
          "typeString": "(compareFn?: (a: T, b: T) => number): T[]"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmpr7dwf": {
      "typeString": "{ (start: number, deleteCount?: number): T[]; (start: number, deleteCount: number, ...items: T[]): T[]; }",
      "entity": "type",
      "id": "T01m4wmpr7dwf",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmyry115"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wn05l5g6"
            ],
            [
              "symbol",
              "S01m4wlytnhrm"
            ]
          ],
          "typeString": "(start: number, deleteCount?: number): T[]"
        },
        {
          "returnType": [
            "type",
            "T01m4wmyry115"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wn05l5g61"
            ],
            [
              "symbol",
              "S01m4wm6yi17k"
            ],
            [
              "symbol",
              "S01m4wnh8h9vm1"
            ]
          ],
          "typeString": "(start: number, deleteCount: number, ...items: T[]): T[]"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlu6056w": {
      "typeString": "(...items: T[]) => number",
      "entity": "type",
      "id": "T01m4wlu6056w",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p302"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnh8h9vm1"
            ]
          ],
          "typeString": "(...items: T[]): number"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmdlgk5v": {
      "typeString": "(searchElement: T, fromIndex?: number) => number",
      "entity": "type",
      "id": "T01m4wmdlgk5v",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p302"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlvty8c4"
            ],
            [
              "symbol",
              "S01m4wmey0hqc"
            ]
          ],
          "typeString": "(searchElement: T, fromIndex?: number): number"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmxyrgh7": {
      "typeString": "(searchElement: T, fromIndex?: number) => number",
      "entity": "type",
      "id": "T01m4wmxyrgh7",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p302"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlvty8c41"
            ],
            [
              "symbol",
              "S01m4wmey0hqc1"
            ]
          ],
          "typeString": "(searchElement: T, fromIndex?: number): number"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm6y98jw": {
      "typeString": "(callbackfn: (value: T, index: number, array: T[]) => boolean, thisArg?: any) => boolean",
      "entity": "type",
      "id": "T01m4wm6y98jw",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4ufg"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlma9c8u"
            ],
            [
              "symbol",
              "S01m4wn5p1brs"
            ]
          ],
          "typeString": "(callbackfn: (value: T, index: number, array: T[]) => boolean, thisArg?: any): boolean"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wndttlkd": {
      "typeString": "(callbackfn: (value: T, index: number, array: T[]) => boolean, thisArg?: any) => boolean",
      "entity": "type",
      "id": "T01m4wndttlkd",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4ufg"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlma9c8u1"
            ],
            [
              "symbol",
              "S01m4wn5p1brs1"
            ]
          ],
          "typeString": "(callbackfn: (value: T, index: number, array: T[]) => boolean, thisArg?: any): boolean"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlwl3hgn": {
      "typeString": "(callbackfn: (value: T, index: number, array: T[]) => void, thisArg?: any) => void",
      "entity": "type",
      "id": "T01m4wlwl3hgn",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmrxy8hi"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlyv2o1t"
            ],
            [
              "symbol",
              "S01m4wn5p1brs1"
            ]
          ],
          "typeString": "(callbackfn: (value: T, index: number, array: T[]) => void, thisArg?: any): void"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm69fnim": {
      "typeString": "<U>(callbackfn: (value: T, index: number, array: T[]) => U, thisArg?: any) => U[]",
      "entity": "type",
      "id": "T01m4wm69fnim",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmfa4g6y"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wlxxxtip"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wn4yyw73"
            ],
            [
              "symbol",
              "S01m4wn5p1brs1"
            ]
          ],
          "typeString": "<U>(callbackfn: (value: T, index: number, array: T[]) => U, thisArg?: any): U[]"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnnni4t7": {
      "typeString": "{ <S extends T>(callbackfn: (value: T, index: number, array: T[]) => value is S, thisArg?: any): S[]; (callbackfn: (value: T, index: number, array: T[]) => any, thisArg?: any): T[]; }",
      "entity": "type",
      "id": "T01m4wnnni4t7",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wnjaskub"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wlxxxrvy"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmptwu19"
            ],
            [
              "symbol",
              "S01m4wn5p1brs1"
            ]
          ],
          "typeString": "<S extends T>(callbackfn: (value: T, index: number, array: T[]) => value is S, thisArg?: any): S[]"
        },
        {
          "returnType": [
            "type",
            "T01m4wmyry115"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmfmogv4"
            ],
            [
              "symbol",
              "S01m4wn5p1brs1"
            ]
          ],
          "typeString": "(callbackfn: (value: T, index: number, array: T[]) => any, thisArg?: any): T[]"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmptq5yi": {
      "typeString": "{ (callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T): T; (callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T, initialValue: T): T; <U>(callbackfn: (previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U, initialValue: U): U; }",
      "entity": "type",
      "id": "T01m4wmptq5yi",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxspc"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmdtedat"
            ]
          ],
          "typeString": "(callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T): T"
        },
        {
          "returnType": [
            "type",
            "T01m4wlxxxspc"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmdtedat1"
            ],
            [
              "symbol",
              "S01m4wmj2jni4"
            ]
          ],
          "typeString": "(callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T, initialValue: T): T"
        },
        {
          "returnType": [
            "type",
            "T01m4wlxxxtip1"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wlxxxtip1"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnntyigb"
            ],
            [
              "symbol",
              "S01m4wmj2jni5"
            ]
          ],
          "typeString": "<U>(callbackfn: (previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U, initialValue: U): U"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmc0jres": {
      "typeString": "{ (callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T): T; (callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T, initialValue: T): T; <U>(callbackfn: (previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U, initialValue: U): U; }",
      "entity": "type",
      "id": "T01m4wmc0jres",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxspc"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmdtedat1"
            ]
          ],
          "typeString": "(callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T): T"
        },
        {
          "returnType": [
            "type",
            "T01m4wlxxxspc"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmdtedat1"
            ],
            [
              "symbol",
              "S01m4wmj2jni41"
            ]
          ],
          "typeString": "(callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T, initialValue: T): T"
        },
        {
          "returnType": [
            "type",
            "T01m4wlxxxtip1"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wlxxxtip1"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnntyigb1"
            ],
            [
              "symbol",
              "S01m4wmj2jni51"
            ]
          ],
          "typeString": "<U>(callbackfn: (previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U, initialValue: U): U"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wntf4ufg": {
      "typeString": "boolean",
      "entity": "type",
      "id": "T01m4wntf4ufg",
      "flags": [
        "Boolean",
        "Union"
      ],
      "primitive": true,
      "types": [
        [
          "type",
          "T01m4wmr2qasi"
        ],
        [
          "type",
          "T01m4wmr2qasi1"
        ]
      ]
    },
    "T01m4wmrxy8hi": {
      "typeString": "void",
      "entity": "type",
      "id": "T01m4wmrxy8hi",
      "flags": [
        "Void"
      ],
      "primitive": true
    },
    "T01m4wmfa4g6y": {
      "typeString": "U[]",
      "entity": "type",
      "id": "T01m4wmfa4g6y",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmyry115"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxtip"
        ]
      ],
      "numberIndexType": [
        "type",
        "T01m4wlxxxtip"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlxxxtip": {
      "typeString": "U",
      "entity": "type",
      "id": "T01m4wlxxxtip",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wlxxxtip1"
        ]
      ]
    },
    "T01m4wnjaskub": {
      "typeString": "S[]",
      "entity": "type",
      "id": "T01m4wnjaskub",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmyry115"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxrvy"
        ]
      ],
      "numberIndexType": [
        "type",
        "T01m4wlxxxrvy"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlxxxrvy": {
      "typeString": "S",
      "entity": "type",
      "id": "T01m4wlxxxrvy",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlxxxtip1": {
      "typeString": "U",
      "entity": "type",
      "id": "T01m4wlxxxtip1",
      "flags": [
        "TypeParameter"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wlxxxtip"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmr2qasi": {
      "typeString": "false",
      "entity": "type",
      "id": "T01m4wmr2qasi",
      "flags": [
        "BooleanLiteral"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmr2qasi1"
        ]
      ]
    },
    "T01m4wmr2qasi1": {
      "typeString": "true",
      "entity": "type",
      "id": "T01m4wmr2qasi1",
      "flags": [
        "BooleanLiteral"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmr2qasi"
        ]
      ]
    },
    "T01m4wncldsak": {
      "typeString": "ConcatArray<T>[]",
      "entity": "type",
      "id": "T01m4wncldsak",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmyry115"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmeud51c"
        ]
      ],
      "numberIndexType": [
        "type",
        "T01m4wmeud51c"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnuhajh0": {
      "typeString": "(T | ConcatArray<T>)[]",
      "entity": "type",
      "id": "T01m4wnuhajh0",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmyry115"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wntf4uds"
        ]
      ],
      "numberIndexType": [
        "type",
        "T01m4wntf4uds"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmkaydfq": {
      "typeString": "(a: T, b: T) => number",
      "entity": "type",
      "id": "T01m4wmkaydfq",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p302"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wndon82a"
            ],
            [
              "symbol",
              "S01m4wne8qn47"
            ]
          ],
          "typeString": "(a: T, b: T): number"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmq9kgkw": {
      "typeString": "(value: T, index: number, array: T[]) => boolean",
      "entity": "type",
      "id": "T01m4wmq9kgkw",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4ufg"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c6"
            ],
            [
              "symbol",
              "S01m4wlm4x82i"
            ],
            [
              "symbol",
              "S01m4wncmdbsm"
            ]
          ],
          "typeString": "(value: T, index: number, array: T[]): boolean"
        }
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmq9kgkw1"
        ]
      ]
    },
    "T01m4wmr2p2yu": {
      "typeString": "any",
      "entity": "type",
      "id": "T01m4wmr2p2yu",
      "flags": [
        "Any"
      ]
    },
    "T01m4wmq9kgkw1": {
      "typeString": "(value: T, index: number, array: T[]) => boolean",
      "entity": "type",
      "id": "T01m4wmq9kgkw1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmq9kgkw"
        ]
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4ufg"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c61"
            ],
            [
              "symbol",
              "S01m4wlm4x82i1"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(value: T, index: number, array: T[]): boolean"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm5hccih": {
      "typeString": "(value: T, index: number, array: T[]) => void",
      "entity": "type",
      "id": "T01m4wm5hccih",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmrxy8hi"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c61"
            ],
            [
              "symbol",
              "S01m4wlm4x82i1"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(value: T, index: number, array: T[]): void"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnqlfugt": {
      "typeString": "(value: T, index: number, array: T[]) => U",
      "entity": "type",
      "id": "T01m4wnqlfugt",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxtip"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c61"
            ],
            [
              "symbol",
              "S01m4wlm4x82i1"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(value: T, index: number, array: T[]): U"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmxibt6a": {
      "typeString": "(value: T, index: number, array: T[]) => value is S",
      "entity": "type",
      "id": "T01m4wmxibt6a",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4ufg"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c61"
            ],
            [
              "symbol",
              "S01m4wlm4x82i1"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(value: T, index: number, array: T[]): value is S"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wn1idoxr": {
      "typeString": "(value: T, index: number, array: T[]) => any",
      "entity": "type",
      "id": "T01m4wn1idoxr",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yu"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c61"
            ],
            [
              "symbol",
              "S01m4wlm4x82i1"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(value: T, index: number, array: T[]): any"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmj274hm": {
      "typeString": "(previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T",
      "entity": "type",
      "id": "T01m4wmj274hm",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxspc"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmv6ma3j"
            ],
            [
              "symbol",
              "S01m4wmqxkkfp"
            ],
            [
              "symbol",
              "S01m4wnojc7xk"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(previousValue: T, currentValue: T, currentIndex: number, array: T[]): T"
        }
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmj274hm1"
        ]
      ]
    },
    "T01m4wmj274hm1": {
      "typeString": "(previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T",
      "entity": "type",
      "id": "T01m4wmj274hm1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmj274hm"
        ]
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxspc"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmv6ma3j1"
            ],
            [
              "symbol",
              "S01m4wmqxkkfp1"
            ],
            [
              "symbol",
              "S01m4wnojc7xk1"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(previousValue: T, currentValue: T, currentIndex: number, array: T[]): T"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wltjfi8l": {
      "typeString": "(previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U",
      "entity": "type",
      "id": "T01m4wltjfi8l",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxtip1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmv6ma3k"
            ],
            [
              "symbol",
              "S01m4wmqxkkfp1"
            ],
            [
              "symbol",
              "S01m4wnojc7xk1"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(previousValue: U, currentValue: T, currentIndex: number, array: T[]): U"
        }
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wltjfi8l1"
        ]
      ]
    },
    "T01m4wltjfi8l1": {
      "typeString": "(previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U",
      "entity": "type",
      "id": "T01m4wltjfi8l1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wltjfi8l"
        ]
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlxxxtip1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmv6ma3k1"
            ],
            [
              "symbol",
              "S01m4wmqxkkfp1"
            ],
            [
              "symbol",
              "S01m4wnojc7xk1"
            ],
            [
              "symbol",
              "S01m4wncmdbsm1"
            ]
          ],
          "typeString": "(previousValue: U, currentValue: T, currentIndex: number, array: T[]): U"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmeud51c": {
      "typeString": "ConcatArray<T>",
      "entity": "type",
      "id": "T01m4wmeud51c",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmeud51c1"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "numberIndexType": [
        "type",
        "T01m4wlxxxspc"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmeud51c1"
        ]
      ]
    },
    "T01m4wntf4uds": {
      "typeString": "T | ConcatArray<T>",
      "entity": "type",
      "id": "T01m4wntf4uds",
      "flags": [
        "Union"
      ],
      "types": [
        [
          "type",
          "T01m4wlxxxspc"
        ],
        [
          "type",
          "T01m4wmeud51c"
        ]
      ]
    },
    "T01m4wmeud51c1": {
      "typeString": "ConcatArray<T>",
      "entity": "type",
      "id": "T01m4wmeud51c1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmeud51c"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmeud51c1"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc1"
        ],
        [
          "type",
          "T01m4wmpo5y1v"
        ]
      ],
      "numberIndexType": [
        "type",
        "T01m4wlxxxspc1"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlxxxspc1": {
      "typeString": "T",
      "entity": "type",
      "id": "T01m4wlxxxspc1",
      "flags": [
        "TypeParameter"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmpo5y1v": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wmpo5y1v",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wmeud51c1"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnfwxooe": {
      "typeString": "(separator?: string) => string",
      "entity": "type",
      "id": "T01m4wnfwxooe",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wndu3pqv1"
            ]
          ],
          "typeString": "(separator?: string): string"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wne7if8m": {
      "typeString": "(start?: number, end?: number) => T[]",
      "entity": "type",
      "id": "T01m4wne7if8m",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmyry1151"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnkh8lv31"
            ],
            [
              "symbol",
              "S01m4wmhq6m6y1"
            ]
          ],
          "typeString": "(start?: number, end?: number): T[]"
        }
      ],
      "libName": "lib.es5.d.ts"
    }
  },
  "symbols": {
    "S01m4wmbiopcd": {
      "id": "S01m4wmbiopcd",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wnvl28mi"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "SimpleClass": [
          "symbol",
          "S01m4wmy6qxnn"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3s32"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3s32"
        ],
        1,
        1,
        5,
        1
      ]
    },
    "S01m4wmy6qxnn": {
      "id": "S01m4wmy6qxnn",
      "entity": "symbol",
      "name": "SimpleClass",
      "flags": [
        "Class"
      ],
      "type": [
        "type",
        "T01m4wlocrpyg"
      ],
      "symbolString": "SimpleClass",
      "typeString": "typeof SimpleClass",
      "members": {
        "foo": [
          "symbol",
          "S01m4wm5niuah"
        ],
        "baz": [
          "symbol",
          "S01m4wmy01yu1"
        ]
      },
      "modifiers": [
        "export"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3s32"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3s32"
        ],
        1,
        1,
        5,
        1
      ]
    },
    "S01m4wm5niuah": {
      "id": "S01m4wm5niuah",
      "entity": "symbol",
      "name": "foo",
      "flags": [
        "Property"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "foo",
      "typeString": "string",
      "modifiers": [
        "public"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3s32"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3s32"
        ],
        3,
        0,
        3,
        29
      ]
    },
    "S01m4wmy01yu1": {
      "id": "S01m4wmy01yu1",
      "entity": "symbol",
      "name": "baz",
      "flags": [
        "Method"
      ],
      "type": [
        "type",
        "T01m4wmt920g7"
      ],
      "symbolString": "baz",
      "typeString": "(x: number[]) => number",
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3s32"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3s32"
        ],
        4,
        0,
        4,
        59
      ]
    },
    "S01m4wlnrhn38": {
      "id": "S01m4wlnrhn38",
      "entity": "symbol",
      "name": "bar",
      "flags": [
        "FunctionScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "bar",
      "typeString": "string",
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3s32"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3s32"
        ],
        2,
        15,
        2,
        25
      ]
    },
    "S01m4wmgai7ni": {
      "id": "S01m4wmgai7ni",
      "entity": "symbol",
      "name": "x",
      "flags": [
        "FunctionScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wlstx004"
      ],
      "symbolString": "x",
      "typeString": "number[]",
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3s32"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3s32"
        ],
        4,
        7,
        4,
        17
      ]
    },
    "S01m4wlljkcrg": {
      "id": "S01m4wlljkcrg",
      "entity": "symbol",
      "name": "length",
      "flags": [
        "Property",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "length",
      "typeString": "number"
    },
    "S01m4wnclba08": {
      "id": "S01m4wnclba08",
      "entity": "symbol",
      "name": "toString",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlmgaa33"
      ],
      "symbolString": "toString",
      "typeString": "() => string"
    },
    "S01m4wlml65d9": {
      "id": "S01m4wlml65d9",
      "entity": "symbol",
      "name": "toLocaleString",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm6hyl1v"
      ],
      "symbolString": "toLocaleString",
      "typeString": "() => string"
    },
    "S01m4wnh2nepy": {
      "id": "S01m4wnh2nepy",
      "entity": "symbol",
      "name": "pop",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmbejkoe"
      ],
      "symbolString": "pop",
      "typeString": "() => T"
    },
    "S01m4wm35aop6": {
      "id": "S01m4wm35aop6",
      "entity": "symbol",
      "name": "push",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlo5jwbl"
      ],
      "symbolString": "push",
      "typeString": "(...items: T[]) => number"
    },
    "S01m4wnefiihw": {
      "id": "S01m4wnefiihw",
      "entity": "symbol",
      "name": "concat",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm9hh1pr"
      ],
      "symbolString": "concat",
      "typeString": "{ (...items: ConcatArray<T>[]): T[]; (...items: (T | ConcatArray<T>)[]): T[]; }"
    },
    "S01m4wngkltv8": {
      "id": "S01m4wngkltv8",
      "entity": "symbol",
      "name": "join",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnw50elx"
      ],
      "symbolString": "join",
      "typeString": "(separator?: string) => string",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wngkltv81"
        ]
      ]
    },
    "S01m4wmuy9l2g": {
      "id": "S01m4wmuy9l2g",
      "entity": "symbol",
      "name": "reverse",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnuhc3nk"
      ],
      "symbolString": "reverse",
      "typeString": "() => T[]"
    },
    "S01m4wmiyhx7n": {
      "id": "S01m4wmiyhx7n",
      "entity": "symbol",
      "name": "shift",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnsu5n5g"
      ],
      "symbolString": "shift",
      "typeString": "() => T"
    },
    "S01m4wlwouxwt": {
      "id": "S01m4wlwouxwt",
      "entity": "symbol",
      "name": "slice",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm3g2s0o"
      ],
      "symbolString": "slice",
      "typeString": "(start?: number, end?: number) => T[]",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlwouxwt1"
        ]
      ]
    },
    "S01m4wmgpbbip": {
      "id": "S01m4wmgpbbip",
      "entity": "symbol",
      "name": "sort",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm5nwh0x"
      ],
      "symbolString": "sort",
      "typeString": "(compareFn?: (a: T, b: T) => number) => T[]"
    },
    "S01m4wmf5napj": {
      "id": "S01m4wmf5napj",
      "entity": "symbol",
      "name": "splice",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmpr7dwf"
      ],
      "symbolString": "splice",
      "typeString": "{ (start: number, deleteCount?: number): T[]; (start: number, deleteCount: number, ...items: T[]): T[]; }"
    },
    "S01m4wmatwspb": {
      "id": "S01m4wmatwspb",
      "entity": "symbol",
      "name": "unshift",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlu6056w"
      ],
      "symbolString": "unshift",
      "typeString": "(...items: T[]) => number"
    },
    "S01m4wmuhwr2t": {
      "id": "S01m4wmuhwr2t",
      "entity": "symbol",
      "name": "indexOf",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmdlgk5v"
      ],
      "symbolString": "indexOf",
      "typeString": "(searchElement: T, fromIndex?: number) => number"
    },
    "S01m4wmoc9yl5": {
      "id": "S01m4wmoc9yl5",
      "entity": "symbol",
      "name": "lastIndexOf",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmxyrgh7"
      ],
      "symbolString": "lastIndexOf",
      "typeString": "(searchElement: T, fromIndex?: number) => number"
    },
    "S01m4wm17yufo": {
      "id": "S01m4wm17yufo",
      "entity": "symbol",
      "name": "every",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm6y98jw"
      ],
      "symbolString": "every",
      "typeString": "(callbackfn: (value: T, index: number, array: T[]) => boolean, thisArg?: any) => boolean"
    },
    "S01m4wmn5alt3": {
      "id": "S01m4wmn5alt3",
      "entity": "symbol",
      "name": "some",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wndttlkd"
      ],
      "symbolString": "some",
      "typeString": "(callbackfn: (value: T, index: number, array: T[]) => boolean, thisArg?: any) => boolean"
    },
    "S01m4wlnp2rtw": {
      "id": "S01m4wlnp2rtw",
      "entity": "symbol",
      "name": "forEach",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlwl3hgn"
      ],
      "symbolString": "forEach",
      "typeString": "(callbackfn: (value: T, index: number, array: T[]) => void, thisArg?: any) => void"
    },
    "S01m4wm6fmeva": {
      "id": "S01m4wm6fmeva",
      "entity": "symbol",
      "name": "map",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm69fnim"
      ],
      "symbolString": "map",
      "typeString": "<U>(callbackfn: (value: T, index: number, array: T[]) => U, thisArg?: any) => U[]"
    },
    "S01m4wlquekh0": {
      "id": "S01m4wlquekh0",
      "entity": "symbol",
      "name": "filter",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnnni4t7"
      ],
      "symbolString": "filter",
      "typeString": "{ <S extends T>(callbackfn: (value: T, index: number, array: T[]) => value is S, thisArg?: any): S[]; (callbackfn: (value: T, index: number, array: T[]) => any, thisArg?: any): T[]; }"
    },
    "S01m4wmlyhthq": {
      "id": "S01m4wmlyhthq",
      "entity": "symbol",
      "name": "reduce",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmptq5yi"
      ],
      "symbolString": "reduce",
      "typeString": "{ (callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T): T; (callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T, initialValue: T): T; <U>(callbackfn: (previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U, initialValue: U): U; }"
    },
    "S01m4wlm4esol": {
      "id": "S01m4wlm4esol",
      "entity": "symbol",
      "name": "reduceRight",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmc0jres"
      ],
      "symbolString": "reduceRight",
      "typeString": "{ (callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T): T; (callbackfn: (previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T, initialValue: T): T; <U>(callbackfn: (previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U, initialValue: U): U; }"
    },
    "S01m4wnh8h9vm": {
      "id": "S01m4wnh8h9vm",
      "entity": "symbol",
      "name": "items",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmyry115"
      ],
      "symbolString": "items",
      "typeString": "T[]",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wnh8h9vm1"
        ]
      ]
    },
    "S01m4wnmco1l5": {
      "id": "S01m4wnmco1l5",
      "entity": "symbol",
      "name": "items",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wncldsak"
      ],
      "symbolString": "items",
      "typeString": "ConcatArray<T>[]"
    },
    "S01m4wniiiax7": {
      "id": "S01m4wniiiax7",
      "entity": "symbol",
      "name": "items",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnuhajh0"
      ],
      "symbolString": "items",
      "typeString": "(T | ConcatArray<T>)[]"
    },
    "S01m4wndu3pqv": {
      "id": "S01m4wndu3pqv",
      "entity": "symbol",
      "name": "separator",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "separator",
      "typeString": "string",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wndu3pqv1"
        ]
      ]
    },
    "S01m4wnkh8lv3": {
      "id": "S01m4wnkh8lv3",
      "entity": "symbol",
      "name": "start",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "start",
      "typeString": "number",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wnkh8lv31"
        ]
      ]
    },
    "S01m4wmhq6m6y": {
      "id": "S01m4wmhq6m6y",
      "entity": "symbol",
      "name": "end",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "end",
      "typeString": "number",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmhq6m6y1"
        ]
      ]
    },
    "S01m4wnsv0e1b": {
      "id": "S01m4wnsv0e1b",
      "entity": "symbol",
      "name": "compareFn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmkaydfq"
      ],
      "symbolString": "compareFn",
      "typeString": "(a: T, b: T) => number"
    },
    "S01m4wn05l5g6": {
      "id": "S01m4wn05l5g6",
      "entity": "symbol",
      "name": "start",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "start",
      "typeString": "number",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wn05l5g61"
        ]
      ]
    },
    "S01m4wlytnhrm": {
      "id": "S01m4wlytnhrm",
      "entity": "symbol",
      "name": "deleteCount",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "deleteCount",
      "typeString": "number"
    },
    "S01m4wn05l5g61": {
      "id": "S01m4wn05l5g61",
      "entity": "symbol",
      "name": "start",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wn05l5g6"
        ]
      ],
      "symbolString": "start",
      "typeString": "number"
    },
    "S01m4wm6yi17k": {
      "id": "S01m4wm6yi17k",
      "entity": "symbol",
      "name": "deleteCount",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "deleteCount",
      "typeString": "number"
    },
    "S01m4wnh8h9vm1": {
      "id": "S01m4wnh8h9vm1",
      "entity": "symbol",
      "name": "items",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmyry115"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wnh8h9vm"
        ]
      ],
      "symbolString": "items",
      "typeString": "T[]"
    },
    "S01m4wlvty8c4": {
      "id": "S01m4wlvty8c4",
      "entity": "symbol",
      "name": "searchElement",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "searchElement",
      "typeString": "T",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlvty8c41"
        ]
      ]
    },
    "S01m4wmey0hqc": {
      "id": "S01m4wmey0hqc",
      "entity": "symbol",
      "name": "fromIndex",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "fromIndex",
      "typeString": "number",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmey0hqc1"
        ]
      ]
    },
    "S01m4wlvty8c41": {
      "id": "S01m4wlvty8c41",
      "entity": "symbol",
      "name": "searchElement",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlvty8c4"
        ]
      ],
      "symbolString": "searchElement",
      "typeString": "T"
    },
    "S01m4wmey0hqc1": {
      "id": "S01m4wmey0hqc1",
      "entity": "symbol",
      "name": "fromIndex",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmey0hqc"
        ]
      ],
      "symbolString": "fromIndex",
      "typeString": "number"
    },
    "S01m4wlma9c8u": {
      "id": "S01m4wlma9c8u",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmq9kgkw"
      ],
      "symbolString": "callbackfn",
      "typeString": "(value: T, index: number, array: T[]) => boolean",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlma9c8u1"
        ]
      ]
    },
    "S01m4wn5p1brs": {
      "id": "S01m4wn5p1brs",
      "entity": "symbol",
      "name": "thisArg",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yu"
      ],
      "symbolString": "thisArg",
      "typeString": "any",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wn5p1brs1"
        ]
      ]
    },
    "S01m4wlma9c8u1": {
      "id": "S01m4wlma9c8u1",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmq9kgkw1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlma9c8u"
        ]
      ],
      "symbolString": "callbackfn",
      "typeString": "(value: T, index: number, array: T[]) => boolean"
    },
    "S01m4wn5p1brs1": {
      "id": "S01m4wn5p1brs1",
      "entity": "symbol",
      "name": "thisArg",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yu"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wn5p1brs"
        ]
      ],
      "symbolString": "thisArg",
      "typeString": "any"
    },
    "S01m4wlyv2o1t": {
      "id": "S01m4wlyv2o1t",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm5hccih"
      ],
      "symbolString": "callbackfn",
      "typeString": "(value: T, index: number, array: T[]) => void"
    },
    "S01m4wn4yyw73": {
      "id": "S01m4wn4yyw73",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnqlfugt"
      ],
      "symbolString": "callbackfn",
      "typeString": "(value: T, index: number, array: T[]) => U"
    },
    "S01m4wmptwu19": {
      "id": "S01m4wmptwu19",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmxibt6a"
      ],
      "symbolString": "callbackfn",
      "typeString": "(value: T, index: number, array: T[]) => value is S"
    },
    "S01m4wmfmogv4": {
      "id": "S01m4wmfmogv4",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wn1idoxr"
      ],
      "symbolString": "callbackfn",
      "typeString": "(value: T, index: number, array: T[]) => any"
    },
    "S01m4wmdtedat": {
      "id": "S01m4wmdtedat",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmj274hm"
      ],
      "symbolString": "callbackfn",
      "typeString": "(previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmdtedat1"
        ]
      ]
    },
    "S01m4wmdtedat1": {
      "id": "S01m4wmdtedat1",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmj274hm1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmdtedat"
        ]
      ],
      "symbolString": "callbackfn",
      "typeString": "(previousValue: T, currentValue: T, currentIndex: number, array: T[]) => T"
    },
    "S01m4wmj2jni4": {
      "id": "S01m4wmj2jni4",
      "entity": "symbol",
      "name": "initialValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "initialValue",
      "typeString": "T",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmj2jni41"
        ]
      ]
    },
    "S01m4wnntyigb": {
      "id": "S01m4wnntyigb",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wltjfi8l"
      ],
      "symbolString": "callbackfn",
      "typeString": "(previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wnntyigb1"
        ]
      ]
    },
    "S01m4wmj2jni5": {
      "id": "S01m4wmj2jni5",
      "entity": "symbol",
      "name": "initialValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxtip1"
      ],
      "symbolString": "initialValue",
      "typeString": "U",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmj2jni51"
        ]
      ]
    },
    "S01m4wmj2jni41": {
      "id": "S01m4wmj2jni41",
      "entity": "symbol",
      "name": "initialValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmj2jni4"
        ]
      ],
      "symbolString": "initialValue",
      "typeString": "T"
    },
    "S01m4wnntyigb1": {
      "id": "S01m4wnntyigb1",
      "entity": "symbol",
      "name": "callbackfn",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wltjfi8l1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wnntyigb"
        ]
      ],
      "symbolString": "callbackfn",
      "typeString": "(previousValue: U, currentValue: T, currentIndex: number, array: T[]) => U"
    },
    "S01m4wmj2jni51": {
      "id": "S01m4wmj2jni51",
      "entity": "symbol",
      "name": "initialValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxtip1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmj2jni5"
        ]
      ],
      "symbolString": "initialValue",
      "typeString": "U"
    },
    "S01m4wndon82a": {
      "id": "S01m4wndon82a",
      "entity": "symbol",
      "name": "a",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "a",
      "typeString": "T"
    },
    "S01m4wne8qn47": {
      "id": "S01m4wne8qn47",
      "entity": "symbol",
      "name": "b",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "b",
      "typeString": "T"
    },
    "S01m4wm68v4c6": {
      "id": "S01m4wm68v4c6",
      "entity": "symbol",
      "name": "value",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "value",
      "typeString": "T",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm68v4c61"
        ]
      ]
    },
    "S01m4wlm4x82i": {
      "id": "S01m4wlm4x82i",
      "entity": "symbol",
      "name": "index",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "index",
      "typeString": "number",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlm4x82i1"
        ]
      ]
    },
    "S01m4wncmdbsm": {
      "id": "S01m4wncmdbsm",
      "entity": "symbol",
      "name": "array",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmyry115"
      ],
      "symbolString": "array",
      "typeString": "T[]",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wncmdbsm1"
        ]
      ]
    },
    "S01m4wm68v4c61": {
      "id": "S01m4wm68v4c61",
      "entity": "symbol",
      "name": "value",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm68v4c6"
        ]
      ],
      "symbolString": "value",
      "typeString": "T"
    },
    "S01m4wlm4x82i1": {
      "id": "S01m4wlm4x82i1",
      "entity": "symbol",
      "name": "index",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlm4x82i"
        ]
      ],
      "symbolString": "index",
      "typeString": "number"
    },
    "S01m4wncmdbsm1": {
      "id": "S01m4wncmdbsm1",
      "entity": "symbol",
      "name": "array",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmyry115"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wncmdbsm"
        ]
      ],
      "symbolString": "array",
      "typeString": "T[]"
    },
    "S01m4wmv6ma3j": {
      "id": "S01m4wmv6ma3j",
      "entity": "symbol",
      "name": "previousValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "previousValue",
      "typeString": "T",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmv6ma3j1"
        ]
      ]
    },
    "S01m4wmqxkkfp": {
      "id": "S01m4wmqxkkfp",
      "entity": "symbol",
      "name": "currentValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "currentValue",
      "typeString": "T",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmqxkkfp1"
        ]
      ]
    },
    "S01m4wnojc7xk": {
      "id": "S01m4wnojc7xk",
      "entity": "symbol",
      "name": "currentIndex",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "currentIndex",
      "typeString": "number",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wnojc7xk1"
        ]
      ]
    },
    "S01m4wmv6ma3j1": {
      "id": "S01m4wmv6ma3j1",
      "entity": "symbol",
      "name": "previousValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmv6ma3j"
        ]
      ],
      "symbolString": "previousValue",
      "typeString": "T"
    },
    "S01m4wmqxkkfp1": {
      "id": "S01m4wmqxkkfp1",
      "entity": "symbol",
      "name": "currentValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmqxkkfp"
        ]
      ],
      "symbolString": "currentValue",
      "typeString": "T"
    },
    "S01m4wnojc7xk1": {
      "id": "S01m4wnojc7xk1",
      "entity": "symbol",
      "name": "currentIndex",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wnojc7xk"
        ]
      ],
      "symbolString": "currentIndex",
      "typeString": "number"
    },
    "S01m4wmv6ma3k": {
      "id": "S01m4wmv6ma3k",
      "entity": "symbol",
      "name": "previousValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxtip1"
      ],
      "symbolString": "previousValue",
      "typeString": "U",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmv6ma3k1"
        ]
      ]
    },
    "S01m4wmv6ma3k1": {
      "id": "S01m4wmv6ma3k1",
      "entity": "symbol",
      "name": "previousValue",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxtip1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmv6ma3k"
        ]
      ],
      "symbolString": "previousValue",
      "typeString": "U"
    },
    "S01m4wmfpcf0h": {
      "id": "S01m4wmfpcf0h",
      "entity": "symbol",
      "name": "length",
      "flags": [
        "Property",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "length",
      "typeString": "number"
    },
    "S01m4wngkltv81": {
      "id": "S01m4wngkltv81",
      "entity": "symbol",
      "name": "join",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnfwxooe"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wngkltv8"
        ]
      ],
      "symbolString": "join",
      "typeString": "(separator?: string) => string"
    },
    "S01m4wlwouxwt1": {
      "id": "S01m4wlwouxwt1",
      "entity": "symbol",
      "name": "slice",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wne7if8m"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlwouxwt"
        ]
      ],
      "symbolString": "slice",
      "typeString": "(start?: number, end?: number) => T[]"
    },
    "S01m4wndu3pqv1": {
      "id": "S01m4wndu3pqv1",
      "entity": "symbol",
      "name": "separator",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wndu3pqv"
        ]
      ],
      "symbolString": "separator",
      "typeString": "string"
    },
    "S01m4wnkh8lv31": {
      "id": "S01m4wnkh8lv31",
      "entity": "symbol",
      "name": "start",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wnkh8lv3"
        ]
      ],
      "symbolString": "start",
      "typeString": "number"
    },
    "S01m4wmhq6m6y1": {
      "id": "S01m4wmhq6m6y1",
      "entity": "symbol",
      "name": "end",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmhq6m6y"
        ]
      ],
      "symbolString": "end",
      "typeString": "number"
    }
  }
}

exports['SerializationSnapshotTests Class with properties, methods and static functions 1'] = {
  "types": {
    "T01m4wn9ycwlm": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wn9ycwlm",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "SimpleClass": [
          "symbol",
          "S01m4wmyrv9k5"
        ]
      }
    },
    "T01m4wnfhvjrp": {
      "typeString": "typeof SimpleClass",
      "entity": "type",
      "id": "T01m4wnfhvjrp",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "hello": [
          "symbol",
          "S01m4wne65t4g"
        ]
      },
      "constructorSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmqoo48u"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlnrhn38"
            ]
          ],
          "typeString": "(bar: string): SimpleClass"
        }
      ]
    },
    "T01m4wmaj70nj": {
      "typeString": "() => string",
      "entity": "type",
      "id": "T01m4wmaj70nj",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "typeString": "(): string"
        }
      ]
    },
    "T01m4wmr2p2yx": {
      "typeString": "string",
      "entity": "type",
      "id": "T01m4wmr2p2yx",
      "flags": [
        "String"
      ],
      "primitive": true
    },
    "T01m4wmqoo48u": {
      "typeString": "SimpleClass",
      "entity": "type",
      "id": "T01m4wmqoo48u",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Class",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqoo48u"
      ],
      "properties": {
        "foo": [
          "symbol",
          "S01m4wm5niuah"
        ]
      },
      "thisType": [
        "type",
        "T01m4wndak457"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmqoo48u1"
        ]
      ]
    },
    "T01m4wndak457": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wndak457",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wmqoo48u1"
      ]
    },
    "T01m4wmqoo48u1": {
      "typeString": "SimpleClass",
      "entity": "type",
      "id": "T01m4wmqoo48u1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmqoo48u"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqoo48u"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wndak457"
        ]
      ]
    }
  },
  "symbols": {
    "S01m4wnmuklwd": {
      "id": "S01m4wnmuklwd",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wn9ycwlm"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "SimpleClass": [
          "symbol",
          "S01m4wmyrv9k5"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3r66"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3r66"
        ],
        1,
        1,
        5,
        1
      ]
    },
    "S01m4wmyrv9k5": {
      "id": "S01m4wmyrv9k5",
      "entity": "symbol",
      "name": "SimpleClass",
      "flags": [
        "Class"
      ],
      "type": [
        "type",
        "T01m4wnfhvjrp"
      ],
      "symbolString": "SimpleClass",
      "typeString": "typeof SimpleClass",
      "exports": {
        "hello": [
          "symbol",
          "S01m4wne65t4g"
        ]
      },
      "members": {
        "foo": [
          "symbol",
          "S01m4wm5niuah"
        ]
      },
      "modifiers": [
        "export"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3r66"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3r66"
        ],
        1,
        1,
        5,
        1
      ]
    },
    "S01m4wne65t4g": {
      "id": "S01m4wne65t4g",
      "entity": "symbol",
      "name": "hello",
      "flags": [
        "Method"
      ],
      "type": [
        "type",
        "T01m4wmaj70nj"
      ],
      "symbolString": "hello",
      "typeString": "() => string",
      "modifiers": [
        "static"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3r66"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3r66"
        ],
        4,
        0,
        4,
        44
      ]
    },
    "S01m4wm5niuah": {
      "id": "S01m4wm5niuah",
      "entity": "symbol",
      "name": "foo",
      "flags": [
        "Property"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "foo",
      "typeString": "string",
      "modifiers": [
        "public"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3r66"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3r66"
        ],
        3,
        0,
        3,
        29
      ]
    },
    "S01m4wlnrhn38": {
      "id": "S01m4wlnrhn38",
      "entity": "symbol",
      "name": "bar",
      "flags": [
        "FunctionScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "bar",
      "typeString": "string",
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3r66"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3r66"
        ],
        2,
        15,
        2,
        25
      ]
    }
  }
}

exports['SerializationSnapshotTests Class with properties, methods and static functions using a variety of access modifier keywords 1'] = {
  "types": {
    "T01m4wmks8qk5": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wmks8qk5",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "SimpleClass": [
          "symbol",
          "S01m4wmmum1qj"
        ]
      }
    },
    "T01m4wnd64in4": {
      "typeString": "typeof SimpleClass",
      "entity": "type",
      "id": "T01m4wnd64in4",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "hello": [
          "symbol",
          "S01m4wn1m5fsv"
        ]
      },
      "constructorSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wnm7v6h4"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlnrhn38"
            ]
          ],
          "typeString": "(bar: string): SimpleClass"
        }
      ]
    },
    "T01m4wmtws9u6": {
      "typeString": "() => string",
      "entity": "type",
      "id": "T01m4wmtws9u6",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "typeString": "(): string"
        }
      ]
    },
    "T01m4wmr2p2yx": {
      "typeString": "string",
      "entity": "type",
      "id": "T01m4wmr2p2yx",
      "flags": [
        "String"
      ],
      "primitive": true
    },
    "T01m4wnm7v6h4": {
      "typeString": "SimpleClass",
      "entity": "type",
      "id": "T01m4wnm7v6h4",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Class",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wnm7v6h4"
      ],
      "properties": {
        "foo": [
          "symbol",
          "S01m4wlv23iyf"
        ]
      },
      "thisType": [
        "type",
        "T01m4wnckod0u"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wnm7v6h41"
        ]
      ]
    },
    "T01m4wnckod0u": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wnckod0u",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wnm7v6h41"
      ]
    },
    "T01m4wnm7v6h41": {
      "typeString": "SimpleClass",
      "entity": "type",
      "id": "T01m4wnm7v6h41",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wnm7v6h4"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wnm7v6h4"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wnckod0u"
        ]
      ]
    }
  },
  "symbols": {
    "S01m4wnh3krqf": {
      "id": "S01m4wnh3krqf",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wmks8qk5"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "SimpleClass": [
          "symbol",
          "S01m4wmmum1qj"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3uej"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3uej"
        ],
        1,
        1,
        8,
        1
      ]
    },
    "S01m4wmmum1qj": {
      "id": "S01m4wmmum1qj",
      "entity": "symbol",
      "name": "SimpleClass",
      "flags": [
        "Class"
      ],
      "type": [
        "type",
        "T01m4wnd64in4"
      ],
      "symbolString": "SimpleClass",
      "typeString": "typeof SimpleClass",
      "exports": {
        "hello": [
          "symbol",
          "S01m4wn1m5fsv"
        ]
      },
      "members": {
        "foo": [
          "symbol",
          "S01m4wlv23iyf"
        ]
      },
      "modifiers": [
        "export",
        "FirstContextualKeyword"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3uej"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3uej"
        ],
        1,
        1,
        8,
        1
      ]
    },
    "S01m4wn1m5fsv": {
      "id": "S01m4wn1m5fsv",
      "entity": "symbol",
      "name": "hello",
      "flags": [
        "Method"
      ],
      "type": [
        "type",
        "T01m4wmtws9u6"
      ],
      "symbolString": "hello",
      "typeString": "() => string",
      "modifiers": [
        "protected",
        "static"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3uej"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3uej"
        ],
        2,
        0,
        3,
        54
      ]
    },
    "S01m4wlv23iyf": {
      "id": "S01m4wlv23iyf",
      "entity": "symbol",
      "name": "foo",
      "flags": [
        "Property"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "foo",
      "typeString": "string",
      "modifiers": [
        "protected",
        "readonly"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3uej"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3uej"
        ],
        4,
        0,
        5,
        41
      ]
    },
    "S01m4wlnrhn38": {
      "id": "S01m4wlnrhn38",
      "entity": "symbol",
      "name": "bar",
      "flags": [
        "FunctionScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "bar",
      "typeString": "string",
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3uej"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3uej"
        ],
        7,
        23,
        7,
        33
      ]
    }
  }
}

exports['SerializationSnapshotTests abstract class Vehicle { numWheels: number = 4; abstract drive(): string; } 1'] = {
  "types": {
    "T01m4wm3y7xwt": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wm3y7xwt",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "Vehicle": [
          "symbol",
          "S01m4wmxv41eu"
        ]
      }
    },
    "T01m4wmu9c278": {
      "typeString": "typeof Vehicle",
      "entity": "type",
      "id": "T01m4wmu9c278",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {},
      "constructorSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmb5vf23"
          ],
          "typeString": "(): Vehicle"
        }
      ]
    },
    "T01m4wmr2p302": {
      "typeString": "number",
      "entity": "type",
      "id": "T01m4wmr2p302",
      "flags": [
        "Number"
      ],
      "primitive": true
    },
    "T01m4wnhrlf5a": {
      "typeString": "() => string",
      "entity": "type",
      "id": "T01m4wnhrlf5a",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "typeString": "(): string"
        }
      ]
    },
    "T01m4wmb5vf23": {
      "typeString": "Vehicle",
      "entity": "type",
      "id": "T01m4wmb5vf23",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Class",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmb5vf23"
      ],
      "properties": {
        "numWheels": [
          "symbol",
          "S01m4wmhyivq8"
        ],
        "drive": [
          "symbol",
          "S01m4wmisaglb"
        ]
      },
      "thisType": [
        "type",
        "T01m4wlvl7ayn"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmb5vf231"
        ]
      ]
    },
    "T01m4wmr2p2yx": {
      "typeString": "string",
      "entity": "type",
      "id": "T01m4wmr2p2yx",
      "flags": [
        "String"
      ],
      "primitive": true
    },
    "T01m4wlvl7ayn": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wlvl7ayn",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wmb5vf231"
      ]
    },
    "T01m4wmb5vf231": {
      "typeString": "Vehicle",
      "entity": "type",
      "id": "T01m4wmb5vf231",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmb5vf23"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmb5vf23"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlvl7ayn"
        ]
      ]
    }
  },
  "symbols": {
    "S01m4wnatn4ip": {
      "id": "S01m4wnatn4ip",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wm3y7xwt"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "Vehicle": [
          "symbol",
          "S01m4wmxv41eu"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n80"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n80"
        ],
        1,
        1,
        5,
        1
      ]
    },
    "S01m4wmxv41eu": {
      "id": "S01m4wmxv41eu",
      "entity": "symbol",
      "name": "Vehicle",
      "flags": [
        "Class"
      ],
      "type": [
        "type",
        "T01m4wmu9c278"
      ],
      "symbolString": "Vehicle",
      "typeString": "typeof Vehicle",
      "members": {
        "numWheels": [
          "symbol",
          "S01m4wmhyivq8"
        ],
        "drive": [
          "symbol",
          "S01m4wmisaglb"
        ]
      },
      "modifiers": [
        "export",
        "FirstContextualKeyword"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n80"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n80"
        ],
        1,
        1,
        5,
        1
      ]
    },
    "S01m4wmhyivq8": {
      "id": "S01m4wmhyivq8",
      "entity": "symbol",
      "name": "numWheels",
      "flags": [
        "Property"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "numWheels",
      "typeString": "number",
      "modifiers": [
        "public"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n80"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n80"
        ],
        2,
        0,
        2,
        31
      ]
    },
    "S01m4wmisaglb": {
      "id": "S01m4wmisaglb",
      "entity": "symbol",
      "name": "drive",
      "flags": [
        "Method"
      ],
      "type": [
        "type",
        "T01m4wnhrlf5a"
      ],
      "symbolString": "drive",
      "typeString": "() => string",
      "modifiers": [
        "public",
        "FirstContextualKeyword"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n80"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n80"
        ],
        3,
        0,
        4,
        34
      ]
    }
  }
}

exports['SerializationSnapshotTests class Vehicle { numWheels: number = 4; drive() { return "vroom";} } 1'] = {
  "types": {
    "T01m4wn7v0f21": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wn7v0f21",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "Vehicle": [
          "symbol",
          "S01m4wnux5pba"
        ]
      }
    },
    "T01m4wnwgm39y": {
      "typeString": "typeof Vehicle",
      "entity": "type",
      "id": "T01m4wnwgm39y",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {},
      "constructorSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wlum8wjm"
          ],
          "typeString": "(): Vehicle"
        }
      ]
    },
    "T01m4wmr2p302": {
      "typeString": "number",
      "entity": "type",
      "id": "T01m4wmr2p302",
      "flags": [
        "Number"
      ],
      "primitive": true
    },
    "T01m4wmrd30df": {
      "typeString": "() => string",
      "entity": "type",
      "id": "T01m4wmrd30df",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "typeString": "(): string"
        }
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmrd30df1"
        ]
      ]
    },
    "T01m4wlum8wjm": {
      "typeString": "Vehicle",
      "entity": "type",
      "id": "T01m4wlum8wjm",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Class",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlum8wjm"
      ],
      "properties": {
        "numWheels": [
          "symbol",
          "S01m4wmhyivq8"
        ],
        "drive": [
          "symbol",
          "S01m4wmlmuayh1"
        ]
      },
      "thisType": [
        "type",
        "T01m4wnfjndq3"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wlum8wjm1"
        ]
      ]
    },
    "T01m4wmr2p2yx": {
      "typeString": "string",
      "entity": "type",
      "id": "T01m4wmr2p2yx",
      "flags": [
        "String"
      ],
      "primitive": true
    },
    "T01m4wnfjndq3": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wnfjndq3",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wlum8wjm1"
      ]
    },
    "T01m4wlum8wjm1": {
      "typeString": "Vehicle",
      "entity": "type",
      "id": "T01m4wlum8wjm1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wlum8wjm"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlum8wjm"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wnfjndq3"
        ]
      ]
    },
    "T01m4wmrd30df1": {
      "typeString": "() => string",
      "entity": "type",
      "id": "T01m4wmrd30df1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmrd30df"
        ]
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "typeString": "(): string"
        }
      ]
    }
  },
  "symbols": {
    "S01m4wnvbn9lk": {
      "id": "S01m4wnvbn9lk",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wn7v0f21"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "Vehicle": [
          "symbol",
          "S01m4wnux5pba"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n74"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n74"
        ],
        1,
        1,
        7,
        1
      ]
    },
    "S01m4wnux5pba": {
      "id": "S01m4wnux5pba",
      "entity": "symbol",
      "name": "Vehicle",
      "flags": [
        "Class"
      ],
      "type": [
        "type",
        "T01m4wnwgm39y"
      ],
      "symbolString": "Vehicle",
      "typeString": "typeof Vehicle",
      "members": {
        "numWheels": [
          "symbol",
          "S01m4wmhyivq8"
        ],
        "drive": [
          "symbol",
          "S01m4wmlmuayh"
        ]
      },
      "modifiers": [
        "export"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n74"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n74"
        ],
        1,
        1,
        7,
        1
      ]
    },
    "S01m4wmhyivq8": {
      "id": "S01m4wmhyivq8",
      "entity": "symbol",
      "name": "numWheels",
      "flags": [
        "Property"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "numWheels",
      "typeString": "number",
      "modifiers": [
        "public"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n74"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n74"
        ],
        2,
        0,
        2,
        31
      ]
    },
    "S01m4wmlmuayh": {
      "id": "S01m4wmlmuayh",
      "entity": "symbol",
      "name": "drive",
      "flags": [
        "Method"
      ],
      "type": [
        "type",
        "T01m4wmrd30df"
      ],
      "symbolString": "drive",
      "typeString": "() => string",
      "modifiers": [
        "public"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n74"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n74"
        ],
        3,
        0,
        6,
        3
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmlmuayh1"
        ]
      ]
    },
    "S01m4wmlmuayh1": {
      "id": "S01m4wmlmuayh1",
      "entity": "symbol",
      "name": "drive",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmrd30df1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wmlmuayh"
        ]
      ],
      "symbolString": "drive",
      "typeString": "() => string",
      "modifiers": [
        "public"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wmlw3n74"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wmlw3n74"
        ],
        3,
        0,
        6,
        3
      ]
    }
  }
}

exports['SerializationSnapshotTests const p: Promise<number> = Promise.resolve(4); 1'] = {
  "types": {
    "T01m4wmqijhej": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wmqijhej",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "p": [
          "symbol",
          "S01m4wnfaa62p"
        ]
      }
    },
    "T01m4wm50lj61": {
      "typeString": "Promise<number>",
      "entity": "type",
      "id": "T01m4wm50lj61",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmr2p302"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlqj73b8": {
      "typeString": "Promise<T>",
      "entity": "type",
      "id": "T01m4wlqj73b8",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Interface",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "properties": {
        "then": [
          "symbol",
          "S01m4wls3tys2"
        ],
        "catch": [
          "symbol",
          "S01m4wn25vj1r"
        ]
      },
      "thisType": [
        "type",
        "T01m4wmkksc2o"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wlqj73b81"
        ]
      ]
    },
    "T01m4wmr2p302": {
      "typeString": "number",
      "entity": "type",
      "id": "T01m4wmr2p302",
      "flags": [
        "Number"
      ],
      "primitive": true
    },
    "T01m4wlxxxspc": {
      "typeString": "T",
      "entity": "type",
      "id": "T01m4wlxxxspc",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wlxxxspc1"
        ]
      ]
    },
    "T01m4wmkksc2o": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wmkksc2o",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wlqj73b81"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlqj73b81": {
      "typeString": "Promise<T>",
      "entity": "type",
      "id": "T01m4wlqj73b81",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wlqj73b8"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc"
        ],
        [
          "type",
          "T01m4wmkksc2o"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnk1emqd": {
      "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>) => Promise<TResult1 | TResult2>",
      "entity": "type",
      "id": "T01m4wnk1emqd",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wn13klmg"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wmkne1m9"
            ],
            [
              "type",
              "T01m4wmc0qq7j"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlld5t7d"
            ],
            [
              "symbol",
              "S01m4wn8jb9ke"
            ]
          ],
          "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>): Promise<TResult1 | TResult2>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlypk2eh": {
      "typeString": "<TResult = never>(onrejected?: (reason: any) => TResult | PromiseLike<TResult>) => Promise<T | TResult>",
      "entity": "type",
      "id": "T01m4wlypk2eh",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmdqhw17"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wmrbt197"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnog26q7"
            ]
          ],
          "typeString": "<TResult = never>(onrejected?: (reason: any) => TResult | PromiseLike<TResult>): Promise<T | TResult>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wn13klmg": {
      "typeString": "Promise<TResult1 | TResult2>",
      "entity": "type",
      "id": "T01m4wn13klmg",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wntf4uds"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmkne1m9": {
      "typeString": "TResult1",
      "entity": "type",
      "id": "T01m4wmkne1m9",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmkne1m91"
        ]
      ]
    },
    "T01m4wmc0qq7j": {
      "typeString": "TResult2",
      "entity": "type",
      "id": "T01m4wmc0qq7j",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmc0qq7j1"
        ]
      ]
    },
    "T01m4wmdqhw17": {
      "typeString": "Promise<T | TResult>",
      "entity": "type",
      "id": "T01m4wmdqhw17",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wntf4uds1"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmrbt197": {
      "typeString": "TResult",
      "entity": "type",
      "id": "T01m4wmrbt197",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wntf4uds": {
      "typeString": "TResult1 | TResult2",
      "entity": "type",
      "id": "T01m4wntf4uds",
      "flags": [
        "Union"
      ],
      "types": [
        [
          "type",
          "T01m4wmkne1m9"
        ],
        [
          "type",
          "T01m4wmc0qq7j"
        ]
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wntf4uds1"
        ]
      ]
    },
    "T01m4wntf4uds1": {
      "typeString": "TResult2 | PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wntf4uds1",
      "flags": [
        "Union"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wntf4uds"
        ]
      ],
      "types": [
        [
          "type",
          "T01m4wmc0qq7j1"
        ],
        [
          "type",
          "T01m4wmyl8t1o1"
        ]
      ]
    },
    "T01m4wm84q08t": {
      "typeString": "(value: T) => TResult1 | PromiseLike<TResult1>",
      "entity": "type",
      "id": "T01m4wm84q08t",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c6"
            ]
          ],
          "typeString": "(value: T): TResult1 | PromiseLike<TResult1>"
        }
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wm84q08t1"
        ]
      ]
    },
    "T01m4wni8bkr7": {
      "typeString": "(reason: any) => TResult2 | PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wni8bkr7",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm5bt10r"
            ]
          ],
          "typeString": "(reason: any): TResult2 | PromiseLike<TResult2>"
        }
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wni8bkr71"
        ]
      ]
    },
    "T01m4wnly0o99": {
      "typeString": "(reason: any) => TResult | PromiseLike<TResult>",
      "entity": "type",
      "id": "T01m4wnly0o99",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm5bt10r1"
            ]
          ],
          "typeString": "(reason: any): TResult | PromiseLike<TResult>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wn56r1oq": {
      "typeString": "PromiseLike<TResult1>",
      "entity": "type",
      "id": "T01m4wn56r1oq",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmkne1m9"
        ]
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wn56r1oq1"
        ]
      ]
    },
    "T01m4wmyl8t1o": {
      "typeString": "PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wmyl8t1o",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmc0qq7j"
        ]
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmyl8t1o1"
        ]
      ]
    },
    "T01m4wncx6qg3": {
      "typeString": "PromiseLike<TResult>",
      "entity": "type",
      "id": "T01m4wncx6qg3",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmrbt197"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmqd5ykg": {
      "typeString": "PromiseLike<T>",
      "entity": "type",
      "id": "T01m4wmqd5ykg",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Interface",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc1"
        ]
      ],
      "properties": {
        "then": [
          "symbol",
          "S01m4wlw3cwyo"
        ]
      },
      "thisType": [
        "type",
        "T01m4wmucvjdh"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmqd5ykg1"
        ]
      ]
    },
    "T01m4wlxxxspc1": {
      "typeString": "T",
      "entity": "type",
      "id": "T01m4wlxxxspc1",
      "flags": [
        "TypeParameter"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmucvjdh": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wmucvjdh",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wmqd5ykg1"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmqd5ykg1": {
      "typeString": "PromiseLike<T>",
      "entity": "type",
      "id": "T01m4wmqd5ykg1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmqd5ykg"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc1"
        ],
        [
          "type",
          "T01m4wmucvjdh"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmr2p2yu": {
      "typeString": "any",
      "entity": "type",
      "id": "T01m4wmr2p2yu",
      "flags": [
        "Any"
      ]
    },
    "T01m4wmik954f": {
      "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>) => PromiseLike<TResult1 | TResult2>",
      "entity": "type",
      "id": "T01m4wmik954f",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wnl3xtw6"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wmkne1m91"
            ],
            [
              "type",
              "T01m4wmc0qq7j1"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlld5t7d1"
            ],
            [
              "symbol",
              "S01m4wn8jb9ke1"
            ]
          ],
          "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>): PromiseLike<TResult1 | TResult2>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnl3xtw6": {
      "typeString": "PromiseLike<TResult1 | TResult2>",
      "entity": "type",
      "id": "T01m4wnl3xtw6",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wntf4uds1"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmkne1m91": {
      "typeString": "TResult1",
      "entity": "type",
      "id": "T01m4wmkne1m91",
      "flags": [
        "TypeParameter"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmkne1m9"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmc0qq7j1": {
      "typeString": "TResult2",
      "entity": "type",
      "id": "T01m4wmc0qq7j1",
      "flags": [
        "TypeParameter"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmc0qq7j"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm84q08t1": {
      "typeString": "(value: T) => TResult1 | PromiseLike<TResult1>",
      "entity": "type",
      "id": "T01m4wm84q08t1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wm84q08t"
        ]
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c61"
            ]
          ],
          "typeString": "(value: T): TResult1 | PromiseLike<TResult1>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wni8bkr71": {
      "typeString": "(reason: any) => TResult2 | PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wni8bkr71",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wni8bkr7"
        ]
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm5bt10r1"
            ]
          ],
          "typeString": "(reason: any): TResult2 | PromiseLike<TResult2>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wn56r1oq1": {
      "typeString": "PromiseLike<TResult1>",
      "entity": "type",
      "id": "T01m4wn56r1oq1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wn56r1oq"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmkne1m91"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmyl8t1o1": {
      "typeString": "PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wmyl8t1o1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmyl8t1o"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmc0qq7j1"
        ]
      ],
      "libName": "lib.es5.d.ts"
    }
  },
  "symbols": {
    "S01m4wluapsp4": {
      "id": "S01m4wluapsp4",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wmqijhej"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "p": [
          "symbol",
          "S01m4wnfaa62p"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyk33"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyk33"
        ],
        1,
        1,
        1,
        53
      ]
    },
    "S01m4wnfaa62p": {
      "id": "S01m4wnfaa62p",
      "entity": "symbol",
      "name": "p",
      "flags": [
        "BlockScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wm50lj61"
      ],
      "symbolString": "p",
      "typeString": "Promise<number>",
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyk33"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyk33"
        ],
        1,
        13,
        1,
        52
      ]
    },
    "S01m4wls3tys2": {
      "id": "S01m4wls3tys2",
      "entity": "symbol",
      "name": "then",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnk1emqd"
      ],
      "symbolString": "then",
      "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>) => Promise<TResult1 | TResult2>"
    },
    "S01m4wn25vj1r": {
      "id": "S01m4wn25vj1r",
      "entity": "symbol",
      "name": "catch",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlypk2eh"
      ],
      "symbolString": "catch",
      "typeString": "<TResult = never>(onrejected?: (reason: any) => TResult | PromiseLike<TResult>) => Promise<T | TResult>"
    },
    "S01m4wlld5t7d": {
      "id": "S01m4wlld5t7d",
      "entity": "symbol",
      "name": "onfulfilled",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm84q08t"
      ],
      "symbolString": "onfulfilled",
      "typeString": "(value: T) => TResult1 | PromiseLike<TResult1>",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlld5t7d1"
        ]
      ]
    },
    "S01m4wn8jb9ke": {
      "id": "S01m4wn8jb9ke",
      "entity": "symbol",
      "name": "onrejected",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wni8bkr7"
      ],
      "symbolString": "onrejected",
      "typeString": "(reason: any) => TResult2 | PromiseLike<TResult2>",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wn8jb9ke1"
        ]
      ]
    },
    "S01m4wnog26q7": {
      "id": "S01m4wnog26q7",
      "entity": "symbol",
      "name": "onrejected",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnly0o99"
      ],
      "symbolString": "onrejected",
      "typeString": "(reason: any) => TResult | PromiseLike<TResult>"
    },
    "S01m4wm68v4c6": {
      "id": "S01m4wm68v4c6",
      "entity": "symbol",
      "name": "value",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "value",
      "typeString": "T",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm68v4c61"
        ]
      ]
    },
    "S01m4wm5bt10r": {
      "id": "S01m4wm5bt10r",
      "entity": "symbol",
      "name": "reason",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yu"
      ],
      "symbolString": "reason",
      "typeString": "any",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm5bt10r1"
        ]
      ]
    },
    "S01m4wm5bt10r1": {
      "id": "S01m4wm5bt10r1",
      "entity": "symbol",
      "name": "reason",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yu"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm5bt10r"
        ]
      ],
      "symbolString": "reason",
      "typeString": "any"
    },
    "S01m4wlw3cwyo": {
      "id": "S01m4wlw3cwyo",
      "entity": "symbol",
      "name": "then",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmik954f"
      ],
      "symbolString": "then",
      "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>) => PromiseLike<TResult1 | TResult2>"
    },
    "S01m4wlld5t7d1": {
      "id": "S01m4wlld5t7d1",
      "entity": "symbol",
      "name": "onfulfilled",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm84q08t1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlld5t7d"
        ]
      ],
      "symbolString": "onfulfilled",
      "typeString": "(value: T) => TResult1 | PromiseLike<TResult1>"
    },
    "S01m4wn8jb9ke1": {
      "id": "S01m4wn8jb9ke1",
      "entity": "symbol",
      "name": "onrejected",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wni8bkr71"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wn8jb9ke"
        ]
      ],
      "symbolString": "onrejected",
      "typeString": "(reason: any) => TResult2 | PromiseLike<TResult2>"
    },
    "S01m4wm68v4c61": {
      "id": "S01m4wm68v4c61",
      "entity": "symbol",
      "name": "value",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm68v4c6"
        ]
      ],
      "symbolString": "value",
      "typeString": "T"
    }
  }
}

exports['SerializationSnapshotTests const x = "foo" 1'] = {
  "types": {
    "T01m4wnq6yv31": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wnq6yv31",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "x": [
          "symbol",
          "S01m4wn2qbwhw"
        ]
      }
    },
    "T01m4wmr2q7oq": {
      "typeString": "\"foo\"",
      "entity": "type",
      "id": "T01m4wmr2q7oq",
      "flags": [
        "StringLiteral"
      ]
    }
  },
  "symbols": {
    "S01m4wm6iq0tu": {
      "id": "S01m4wm6iq0tu",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wnq6yv31"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "x": [
          "symbol",
          "S01m4wn2qbwhw"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyhpp"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyhpp"
        ],
        1,
        1,
        1,
        23
      ]
    },
    "S01m4wn2qbwhw": {
      "id": "S01m4wn2qbwhw",
      "entity": "symbol",
      "name": "x",
      "flags": [
        "BlockScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wmr2q7oq"
      ],
      "symbolString": "x",
      "typeString": "\"foo\"",
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyhpp"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyhpp"
        ],
        1,
        13,
        1,
        22
      ]
    }
  }
}

exports['SerializationSnapshotTests function add(a: number, b: string) { return a + b; } 1'] = {
  "types": {
    "T01m4wms4srob": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wms4srob",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "add": [
          "symbol",
          "S01m4wnmioev0"
        ]
      }
    },
    "T01m4wmqo0i12": {
      "typeString": "(a: number, b: string) => string",
      "entity": "type",
      "id": "T01m4wmqo0i12",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmr2p2yx"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wmpm9hn4"
            ],
            [
              "symbol",
              "S01m4wn6hbne2"
            ]
          ],
          "typeString": "(a: number, b: string): string"
        }
      ]
    },
    "T01m4wmr2p2yx": {
      "typeString": "string",
      "entity": "type",
      "id": "T01m4wmr2p2yx",
      "flags": [
        "String"
      ],
      "primitive": true
    },
    "T01m4wmr2p302": {
      "typeString": "number",
      "entity": "type",
      "id": "T01m4wmr2p302",
      "flags": [
        "Number"
      ],
      "primitive": true
    }
  },
  "symbols": {
    "S01m4wmoyq991": {
      "id": "S01m4wmoyq991",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wms4srob"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "add": [
          "symbol",
          "S01m4wnmioev0"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyk8e"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyk8e"
        ],
        1,
        1,
        1,
        59
      ]
    },
    "S01m4wnmioev0": {
      "id": "S01m4wnmioev0",
      "entity": "symbol",
      "name": "add",
      "flags": [
        "Function"
      ],
      "type": [
        "type",
        "T01m4wmqo0i12"
      ],
      "symbolString": "add",
      "typeString": "(a: number, b: string) => string",
      "modifiers": [
        "export"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyk8e"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyk8e"
        ],
        1,
        1,
        1,
        59
      ]
    },
    "S01m4wmpm9hn4": {
      "id": "S01m4wmpm9hn4",
      "entity": "symbol",
      "name": "a",
      "flags": [
        "FunctionScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "a",
      "typeString": "number",
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyk8e"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyk8e"
        ],
        1,
        21,
        1,
        29
      ]
    },
    "S01m4wn6hbne2": {
      "id": "S01m4wn6hbne2",
      "entity": "symbol",
      "name": "b",
      "flags": [
        "FunctionScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "b",
      "typeString": "string",
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyk8e"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyk8e"
        ],
        1,
        31,
        1,
        40
      ]
    }
  }
}

exports['SerializationSnapshotTests interface Foo {bar: number; readonly baz: Promise<string>} 1'] = {
  "types": {
    "T01m4wm96bh7s": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wm96bh7s",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ]
    },
    "T01m4wmp9klrl": {
      "typeString": "Foo",
      "entity": "type",
      "id": "T01m4wmp9klrl",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Interface"
      ],
      "properties": {
        "bar": [
          "symbol",
          "S01m4wn1knui5"
        ],
        "baz": [
          "symbol",
          "S01m4wmje5qmw"
        ]
      }
    },
    "T01m4wmr2p302": {
      "typeString": "number",
      "entity": "type",
      "id": "T01m4wmr2p302",
      "flags": [
        "Number"
      ],
      "primitive": true
    },
    "T01m4wlxcvbvh": {
      "typeString": "Promise<string>",
      "entity": "type",
      "id": "T01m4wlxcvbvh",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmr2p2yx"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlqj73b8": {
      "typeString": "Promise<T>",
      "entity": "type",
      "id": "T01m4wlqj73b8",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Interface",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "properties": {
        "then": [
          "symbol",
          "S01m4wls3tys2"
        ],
        "catch": [
          "symbol",
          "S01m4wn25vj1r"
        ]
      },
      "thisType": [
        "type",
        "T01m4wmkksc2o"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wlqj73b81"
        ]
      ]
    },
    "T01m4wmr2p2yx": {
      "typeString": "string",
      "entity": "type",
      "id": "T01m4wmr2p2yx",
      "flags": [
        "String"
      ],
      "primitive": true
    },
    "T01m4wlxxxspc": {
      "typeString": "T",
      "entity": "type",
      "id": "T01m4wlxxxspc",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wlxxxspc1"
        ]
      ]
    },
    "T01m4wmkksc2o": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wmkksc2o",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wlqj73b81"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlqj73b81": {
      "typeString": "Promise<T>",
      "entity": "type",
      "id": "T01m4wlqj73b81",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wlqj73b8"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc"
        ],
        [
          "type",
          "T01m4wmkksc2o"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnk1emqd": {
      "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>) => Promise<TResult1 | TResult2>",
      "entity": "type",
      "id": "T01m4wnk1emqd",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wn13klmg"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wmkne1m9"
            ],
            [
              "type",
              "T01m4wmc0qq7j"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlld5t7d"
            ],
            [
              "symbol",
              "S01m4wn8jb9ke"
            ]
          ],
          "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>): Promise<TResult1 | TResult2>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wlypk2eh": {
      "typeString": "<TResult = never>(onrejected?: (reason: any) => TResult | PromiseLike<TResult>) => Promise<T | TResult>",
      "entity": "type",
      "id": "T01m4wlypk2eh",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wmdqhw17"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wmrbt197"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wnog26q7"
            ]
          ],
          "typeString": "<TResult = never>(onrejected?: (reason: any) => TResult | PromiseLike<TResult>): Promise<T | TResult>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wn13klmg": {
      "typeString": "Promise<TResult1 | TResult2>",
      "entity": "type",
      "id": "T01m4wn13klmg",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wntf4uds"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmkne1m9": {
      "typeString": "TResult1",
      "entity": "type",
      "id": "T01m4wmkne1m9",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmkne1m91"
        ]
      ]
    },
    "T01m4wmc0qq7j": {
      "typeString": "TResult2",
      "entity": "type",
      "id": "T01m4wmc0qq7j",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmc0qq7j1"
        ]
      ]
    },
    "T01m4wmdqhw17": {
      "typeString": "Promise<T | TResult>",
      "entity": "type",
      "id": "T01m4wmdqhw17",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wlqj73b8"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wntf4uds1"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmrbt197": {
      "typeString": "TResult",
      "entity": "type",
      "id": "T01m4wmrbt197",
      "flags": [
        "TypeParameter"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wntf4uds": {
      "typeString": "TResult1 | TResult2",
      "entity": "type",
      "id": "T01m4wntf4uds",
      "flags": [
        "Union"
      ],
      "types": [
        [
          "type",
          "T01m4wmkne1m9"
        ],
        [
          "type",
          "T01m4wmc0qq7j"
        ]
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wntf4uds1"
        ]
      ]
    },
    "T01m4wntf4uds1": {
      "typeString": "TResult2 | PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wntf4uds1",
      "flags": [
        "Union"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wntf4uds"
        ]
      ],
      "types": [
        [
          "type",
          "T01m4wmc0qq7j1"
        ],
        [
          "type",
          "T01m4wmyl8t1o1"
        ]
      ]
    },
    "T01m4wm84q08t": {
      "typeString": "(value: T) => TResult1 | PromiseLike<TResult1>",
      "entity": "type",
      "id": "T01m4wm84q08t",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c6"
            ]
          ],
          "typeString": "(value: T): TResult1 | PromiseLike<TResult1>"
        }
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wm84q08t1"
        ]
      ]
    },
    "T01m4wni8bkr7": {
      "typeString": "(reason: any) => TResult2 | PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wni8bkr7",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm5bt10r"
            ]
          ],
          "typeString": "(reason: any): TResult2 | PromiseLike<TResult2>"
        }
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wni8bkr71"
        ]
      ]
    },
    "T01m4wnly0o99": {
      "typeString": "(reason: any) => TResult | PromiseLike<TResult>",
      "entity": "type",
      "id": "T01m4wnly0o99",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm5bt10r1"
            ]
          ],
          "typeString": "(reason: any): TResult | PromiseLike<TResult>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wn56r1oq": {
      "typeString": "PromiseLike<TResult1>",
      "entity": "type",
      "id": "T01m4wn56r1oq",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmkne1m9"
        ]
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wn56r1oq1"
        ]
      ]
    },
    "T01m4wmyl8t1o": {
      "typeString": "PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wmyl8t1o",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmc0qq7j"
        ]
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmyl8t1o1"
        ]
      ]
    },
    "T01m4wncx6qg3": {
      "typeString": "PromiseLike<TResult>",
      "entity": "type",
      "id": "T01m4wncx6qg3",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmrbt197"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmqd5ykg": {
      "typeString": "PromiseLike<T>",
      "entity": "type",
      "id": "T01m4wmqd5ykg",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Interface",
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc1"
        ]
      ],
      "properties": {
        "then": [
          "symbol",
          "S01m4wlw3cwyo"
        ]
      },
      "thisType": [
        "type",
        "T01m4wmucvjdh"
      ],
      "libName": "lib.es5.d.ts",
      "relatedTypes": [
        [
          "type",
          "T01m4wmqd5ykg1"
        ]
      ]
    },
    "T01m4wlxxxspc1": {
      "typeString": "T",
      "entity": "type",
      "id": "T01m4wlxxxspc1",
      "flags": [
        "TypeParameter"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmucvjdh": {
      "typeString": "this",
      "entity": "type",
      "id": "T01m4wmucvjdh",
      "flags": [
        "TypeParameter"
      ],
      "isThisType": true,
      "constraint": [
        "type",
        "T01m4wmqd5ykg1"
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmqd5ykg1": {
      "typeString": "PromiseLike<T>",
      "entity": "type",
      "id": "T01m4wmqd5ykg1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmqd5ykg"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc1"
        ],
        [
          "type",
          "T01m4wmucvjdh"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmr2p2yu": {
      "typeString": "any",
      "entity": "type",
      "id": "T01m4wmr2p2yu",
      "flags": [
        "Any"
      ]
    },
    "T01m4wmik954f": {
      "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>) => PromiseLike<TResult1 | TResult2>",
      "entity": "type",
      "id": "T01m4wmik954f",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wnl3xtw6"
          ],
          "typeParameters": [
            [
              "type",
              "T01m4wmkne1m91"
            ],
            [
              "type",
              "T01m4wmc0qq7j1"
            ]
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wlld5t7d1"
            ],
            [
              "symbol",
              "S01m4wn8jb9ke1"
            ]
          ],
          "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>): PromiseLike<TResult1 | TResult2>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wnl3xtw6": {
      "typeString": "PromiseLike<TResult1 | TResult2>",
      "entity": "type",
      "id": "T01m4wnl3xtw6",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wntf4uds1"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmkne1m91": {
      "typeString": "TResult1",
      "entity": "type",
      "id": "T01m4wmkne1m91",
      "flags": [
        "TypeParameter"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmkne1m9"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmc0qq7j1": {
      "typeString": "TResult2",
      "entity": "type",
      "id": "T01m4wmc0qq7j1",
      "flags": [
        "TypeParameter"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmc0qq7j"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wm84q08t1": {
      "typeString": "(value: T) => TResult1 | PromiseLike<TResult1>",
      "entity": "type",
      "id": "T01m4wm84q08t1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wm84q08t"
        ]
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm68v4c61"
            ]
          ],
          "typeString": "(value: T): TResult1 | PromiseLike<TResult1>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wni8bkr71": {
      "typeString": "(reason: any) => TResult2 | PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wni8bkr71",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wni8bkr7"
        ]
      ],
      "objectFlags": [
        "Anonymous",
        "Instantiated"
      ],
      "callSignatures": [
        {
          "returnType": [
            "type",
            "T01m4wntf4uds1"
          ],
          "parameters": [
            [
              "symbol",
              "S01m4wm5bt10r1"
            ]
          ],
          "typeString": "(reason: any): TResult2 | PromiseLike<TResult2>"
        }
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wn56r1oq1": {
      "typeString": "PromiseLike<TResult1>",
      "entity": "type",
      "id": "T01m4wn56r1oq1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wn56r1oq"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmkne1m91"
        ]
      ],
      "libName": "lib.es5.d.ts"
    },
    "T01m4wmyl8t1o1": {
      "typeString": "PromiseLike<TResult2>",
      "entity": "type",
      "id": "T01m4wmyl8t1o1",
      "flags": [
        "Object"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmyl8t1o"
        ]
      ],
      "objectFlags": [
        "Reference"
      ],
      "target": [
        "type",
        "T01m4wmqd5ykg"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wmc0qq7j1"
        ]
      ],
      "libName": "lib.es5.d.ts"
    }
  },
  "symbols": {
    "S01m4wmxaqaka": {
      "id": "S01m4wmxaqaka",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wm96bh7s"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "default": [
          "symbol",
          "S01m4wlmu7jqa"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyln0"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyln0"
        ],
        1,
        1,
        1,
        73
      ]
    },
    "S01m4wlmu7jqa": {
      "id": "S01m4wlmu7jqa",
      "entity": "symbol",
      "name": "default",
      "flags": [
        "Interface"
      ],
      "type": [
        "type",
        "T01m4wmp9klrl"
      ],
      "symbolString": "Foo",
      "typeString": "Foo",
      "members": {
        "bar": [
          "symbol",
          "S01m4wn1knui5"
        ],
        "baz": [
          "symbol",
          "S01m4wmje5qmw"
        ]
      }
    },
    "S01m4wn1knui5": {
      "id": "S01m4wn1knui5",
      "entity": "symbol",
      "name": "bar",
      "flags": [
        "Property"
      ],
      "type": [
        "type",
        "T01m4wmr2p302"
      ],
      "symbolString": "bar",
      "typeString": "number",
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyln0"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyln0"
        ],
        1,
        31,
        1,
        42
      ]
    },
    "S01m4wmje5qmw": {
      "id": "S01m4wmje5qmw",
      "entity": "symbol",
      "name": "baz",
      "flags": [
        "Property"
      ],
      "type": [
        "type",
        "T01m4wlxcvbvh"
      ],
      "symbolString": "baz",
      "typeString": "Promise<string>",
      "modifiers": [
        "readonly"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyln0"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyln0"
        ],
        1,
        43,
        1,
        72
      ]
    },
    "S01m4wls3tys2": {
      "id": "S01m4wls3tys2",
      "entity": "symbol",
      "name": "then",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnk1emqd"
      ],
      "symbolString": "then",
      "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>) => Promise<TResult1 | TResult2>"
    },
    "S01m4wn25vj1r": {
      "id": "S01m4wn25vj1r",
      "entity": "symbol",
      "name": "catch",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlypk2eh"
      ],
      "symbolString": "catch",
      "typeString": "<TResult = never>(onrejected?: (reason: any) => TResult | PromiseLike<TResult>) => Promise<T | TResult>"
    },
    "S01m4wlld5t7d": {
      "id": "S01m4wlld5t7d",
      "entity": "symbol",
      "name": "onfulfilled",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm84q08t"
      ],
      "symbolString": "onfulfilled",
      "typeString": "(value: T) => TResult1 | PromiseLike<TResult1>",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlld5t7d1"
        ]
      ]
    },
    "S01m4wn8jb9ke": {
      "id": "S01m4wn8jb9ke",
      "entity": "symbol",
      "name": "onrejected",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wni8bkr7"
      ],
      "symbolString": "onrejected",
      "typeString": "(reason: any) => TResult2 | PromiseLike<TResult2>",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wn8jb9ke1"
        ]
      ]
    },
    "S01m4wnog26q7": {
      "id": "S01m4wnog26q7",
      "entity": "symbol",
      "name": "onrejected",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wnly0o99"
      ],
      "symbolString": "onrejected",
      "typeString": "(reason: any) => TResult | PromiseLike<TResult>"
    },
    "S01m4wm68v4c6": {
      "id": "S01m4wm68v4c6",
      "entity": "symbol",
      "name": "value",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc"
      ],
      "symbolString": "value",
      "typeString": "T",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm68v4c61"
        ]
      ]
    },
    "S01m4wm5bt10r": {
      "id": "S01m4wm5bt10r",
      "entity": "symbol",
      "name": "reason",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yu"
      ],
      "symbolString": "reason",
      "typeString": "any",
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm5bt10r1"
        ]
      ]
    },
    "S01m4wm5bt10r1": {
      "id": "S01m4wm5bt10r1",
      "entity": "symbol",
      "name": "reason",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yu"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm5bt10r"
        ]
      ],
      "symbolString": "reason",
      "typeString": "any"
    },
    "S01m4wlw3cwyo": {
      "id": "S01m4wlw3cwyo",
      "entity": "symbol",
      "name": "then",
      "flags": [
        "Method",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wmik954f"
      ],
      "symbolString": "then",
      "typeString": "<TResult1 = T, TResult2 = never>(onfulfilled?: (value: T) => TResult1 | PromiseLike<TResult1>, onrejected?: (reason: any) => TResult2 | PromiseLike<TResult2>) => PromiseLike<TResult1 | TResult2>"
    },
    "S01m4wlld5t7d1": {
      "id": "S01m4wlld5t7d1",
      "entity": "symbol",
      "name": "onfulfilled",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wm84q08t1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wlld5t7d"
        ]
      ],
      "symbolString": "onfulfilled",
      "typeString": "(value: T) => TResult1 | PromiseLike<TResult1>"
    },
    "S01m4wn8jb9ke1": {
      "id": "S01m4wn8jb9ke1",
      "entity": "symbol",
      "name": "onrejected",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wni8bkr71"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wn8jb9ke"
        ]
      ],
      "symbolString": "onrejected",
      "typeString": "(reason: any) => TResult2 | PromiseLike<TResult2>"
    },
    "S01m4wm68v4c61": {
      "id": "S01m4wm68v4c61",
      "entity": "symbol",
      "name": "value",
      "flags": [
        "FunctionScopedVariable",
        "Transient"
      ],
      "type": [
        "type",
        "T01m4wlxxxspc1"
      ],
      "relatedSymbols": [
        [
          "symbol",
          "S01m4wm68v4c6"
        ]
      ],
      "symbolString": "value",
      "typeString": "T"
    }
  }
}

exports['SerializationSnapshotTests let x = "foo" 1'] = {
  "types": {
    "T01m4wlqrgfql": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wlqrgfql",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "properties": {
        "x": [
          "symbol",
          "S01m4wn2qbwhw"
        ]
      }
    },
    "T01m4wmr2p2yx": {
      "typeString": "string",
      "entity": "type",
      "id": "T01m4wmr2p2yx",
      "flags": [
        "String"
      ],
      "primitive": true
    }
  },
  "symbols": {
    "S01m4wlsp5n6j": {
      "id": "S01m4wlsp5n6j",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wlqrgfql"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "x": [
          "symbol",
          "S01m4wn2qbwhw"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyhnx"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyhnx"
        ],
        1,
        1,
        1,
        21
      ]
    },
    "S01m4wn2qbwhw": {
      "id": "S01m4wn2qbwhw",
      "entity": "symbol",
      "name": "x",
      "flags": [
        "BlockScopedVariable"
      ],
      "type": [
        "type",
        "T01m4wmr2p2yx"
      ],
      "symbolString": "x",
      "typeString": "string",
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyhnx"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyhnx"
        ],
        1,
        11,
        1,
        20
      ]
    }
  }
}

exports['SerializationSnapshotTests type Dict<T extends "foo"|"bar"|"baz"> = { [k: string]: T | undefined } 1'] = {
  "types": {
    "T01m4wmqcfkr3": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wmqcfkr3",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ]
    },
    "T01m4wlmpiil5": {
      "typeString": "Dict<T>",
      "entity": "type",
      "id": "T01m4wlmpiil5",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "stringIndexType": [
        "type",
        "T01m4wlxxxspc"
      ]
    },
    "T01m4wlxxxspc": {
      "typeString": "T",
      "entity": "type",
      "id": "T01m4wlxxxspc",
      "flags": [
        "TypeParameter"
      ],
      "constraint": [
        "type",
        "T01m4wntf4uds"
      ]
    },
    "T01m4wntf4uds": {
      "typeString": "\"foo\" | \"bar\" | \"baz\"",
      "entity": "type",
      "id": "T01m4wntf4uds",
      "flags": [
        "Union"
      ],
      "types": [
        [
          "type",
          "T01m4wmr2q7oq"
        ],
        [
          "type",
          "T01m4wmr2q7oq1"
        ],
        [
          "type",
          "T01m4wmr2q7oq1"
        ]
      ]
    },
    "T01m4wmr2q7oq": {
      "typeString": "\"foo\"",
      "entity": "type",
      "id": "T01m4wmr2q7oq",
      "flags": [
        "StringLiteral"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmr2q7oq1"
        ]
      ]
    },
    "T01m4wmr2q7oq1": {
      "typeString": "\"baz\"",
      "entity": "type",
      "id": "T01m4wmr2q7oq1",
      "flags": [
        "StringLiteral"
      ],
      "relatedTypes": [
        [
          "type",
          "T01m4wmr2q7oq"
        ]
      ]
    }
  },
  "symbols": {
    "S01m4wmxl83ne": {
      "id": "S01m4wmxl83ne",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wmqcfkr3"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "Dict": [
          "symbol",
          "S01m4wmnqibyq"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkylrf"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkylrf"
        ],
        1,
        1,
        1,
        78
      ]
    },
    "S01m4wmnqibyq": {
      "id": "S01m4wmnqibyq",
      "entity": "symbol",
      "name": "Dict",
      "flags": [
        "TypeAlias"
      ],
      "type": [
        "type",
        "T01m4wlmpiil5"
      ],
      "symbolString": "Dict",
      "typeString": "Dict<T>",
      "modifiers": [
        "export"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkylrf"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkylrf"
        ],
        1,
        1,
        1,
        78
      ]
    }
  }
}

exports['SerializationSnapshotTests type Dict<T> = { [k: string]: T | undefined } 1'] = {
  "types": {
    "T01m4wlwowu0v": {
      "typeString": "typeof import(\"/src/index\")",
      "entity": "type",
      "id": "T01m4wlwowu0v",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ]
    },
    "T01m4wlmpiil5": {
      "typeString": "Dict<T>",
      "entity": "type",
      "id": "T01m4wlmpiil5",
      "flags": [
        "Object"
      ],
      "objectFlags": [
        "Anonymous"
      ],
      "typeParameters": [
        [
          "type",
          "T01m4wlxxxspc"
        ]
      ],
      "stringIndexType": [
        "type",
        "T01m4wlxxxspc"
      ]
    },
    "T01m4wlxxxspc": {
      "typeString": "T",
      "entity": "type",
      "id": "T01m4wlxxxspc",
      "flags": [
        "TypeParameter"
      ]
    }
  },
  "symbols": {
    "S01m4wn2ca0ld": {
      "id": "S01m4wn2ca0ld",
      "entity": "symbol",
      "name": "\"/src/index\"",
      "flags": [
        "ValueModule"
      ],
      "type": [
        "type",
        "T01m4wlwowu0v"
      ],
      "symbolString": "\"--ROOT PATH--/src/index\"",
      "typeString": "typeof import(\"--ROOT PATH--/src/index\")",
      "exports": {
        "Dict": [
          "symbol",
          "S01m4wmnqibyq"
        ]
      },
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyk27"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyk27"
        ],
        1,
        1,
        1,
        52
      ]
    },
    "S01m4wmnqibyq": {
      "id": "S01m4wmnqibyq",
      "entity": "symbol",
      "name": "Dict",
      "flags": [
        "TypeAlias"
      ],
      "type": [
        "type",
        "T01m4wlmpiil5"
      ],
      "symbolString": "Dict",
      "typeString": "Dict<T>",
      "modifiers": [
        "export"
      ],
      "sourceFile": [
        "sourceFile",
        "F01m4wnwkyk27"
      ],
      "location": [
        [
          "sourceFile",
          "F01m4wnwkyk27"
        ],
        1,
        1,
        1,
        52
      ]
    }
  }
}
